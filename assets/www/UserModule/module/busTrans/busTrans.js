...
//			aigovApp.ajax("UserModule/module/busRefresh/busRealtimeInfo.json",null,function(data){
            aigovApp.ajax(aigovApp.constants.busProperties.BUS_URL+"getNearbyBus?stationName="+currentSelectLocation+"&lineName="+appWindow.intent+"&directions=1",null,function(data){

                busImgDataNo = data.data.length;    //车辆数量
				var template_html = $('#bus_realtime_trans_query_template').html();
				var template = Handlebars.compile(template_html);
				...
				var result_html = template(data);
				 $('#busTrans-query').html(result_html);
			});
	}
    //获取公交线路站点
...
//		 aigovApp.ajax("UserModule/module/busRefresh/busRouteStation.json",null,function(data){
         aigovApp.ajax(aigovApp.constants.busProperties.BUS_URL+"getLineStateByLineName?lineName="+appWindow.intent+"&direction=1",null,function(data){
                 //同站线路塞参数
			    ...
			    var template_html = $('#bus_realtime_trans_station_template').html();
                var template = Handlebars.compile(template_html);
                 //格式化交通线路图
                  ...
                  $('#busTrans-station').html(result_html);
                  //位置偏移函数
    ...
    function setLocationToSameRoute(appWindow,currentLocation) {
     ...
     //点击站点，处理信息
     $(document).on("click", "#bus_trans_station li", function(){
      ...
      function offsetOfLocation(busImgDataNo) {
        	$('#bus_trans_station li').each(function(index,element){
        		var bus_li = $(this);
        		if($(this).children("p").hasClass("p_color")) {    //如果包含pclass
        			...
        			$('#bus_trans_station').scrollLeft(offsetLeft*offsetNum);
...
//        	    if (busImgDataNo == 3) {
//                    $('#bus_trans_station li:nth-of-type('+(index+1)+')').css({
//                        "background-image":"url(UserModule/module/busRealtime/style/bus_realtime.png)",
//                        "background-repeat":"no-repeat",
//                        "background-position":"20% 10%",
//                        "background-size":"1.8rem"
//                    });
//                    $('#bus_trans_station li:nth-of-type('+index+')').css({
//                        "background-image":"url(UserModule/module/busRealtime/style/bus_realtime.png)",
//                        "background-repeat":"no-repeat",
//                        "background-position":"20% 10%",
//                        "background-size":"1.8rem"
//                    });
//                    $('#bus_trans_station li:nth-of-type('+(index-2)+')').css({
//                        "background-image":"url(UserModule/module/busRealtime/style/bus_realtime.png)",
//                        "background-repeat":"no-repeat",
//                        "background-position":"20% 10%",
//                        "background-size":"1.8rem"
