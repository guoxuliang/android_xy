	...
	var exportsMethods = {
		...
		goDefault:function () {
			...
			if(module==""){
				...
				var url=aigovApp.constants.indexProperties.ADVERY_URL;
                aigovApp.ajax(url,null, function(rs){
                    ...
                    if(rs.code==1){
                            var status=rs.data[0].child[0].status;
                            if(status==0){
                                appWindow.open(_this.mainWinId,null,_this.config);
                            }else{
                                appWindow.open(_this.advertWinId,rs,_this.config);
                            }
                        }else{
                            appWindow.open(_this.mainWinId,null,_this.config);
                        }
        ...
        /**
	 	 ...
	 	 */
		init:function(mainWinId,welcomeWinId,advertWinId,config){
			this.mainWinId=mainWinId;
			this.welcomeWinId=welcomeWinId;
			this.config=config;
			this.advertWinId=advertWinId;
			this.goDefault();
			this.hashchange();
		}
